...
const mockProducto = { 
  ...
  stock: 10, 
  categorias: [],
  // Métodos esperados por middleware/itemPedidoCreator.js
  estaDisponible: (cantidad) => true,
  reducirStock: (cantidad) => true
};
...
jest.mock("../models/repositories/productoRepository.js", () => ({
  ...
  default: {
    ...
    // Mocks adicionales requeridos por los tests de pedido
    estaDisponible: jest.fn().mockResolvedValue(true),
    reducirStock: jest.fn().mockResolvedValue(true),
  actualizar: jest.fn().mockImplementation((id, producto) => Promise.resolve(producto)),
    mockFindById: jest.fn((id) => {
...
jest.mock("../models/repositories/pedidoRepository.js", () => ({
  __esModule: true,
  default: {
    crearPedido: jest.fn((pedido) => {
      // Simular la respuesta guardada con un _id y con los items que contienen el producto original
      const items = (pedido.items || []).map(it => ({
        producto: mockProducto,
        cantidad: it.cantidad,
        precioUnitario: it.precioUnitario || mockProducto.precio
      }));
      return Promise.resolve({ ...pedido, _id: 99, items });
    }),
    findById: jest.fn((id) => {
